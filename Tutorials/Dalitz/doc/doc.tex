...
Only one external package is required to run the Dalitz tutorial: {\tt ROOT}. Once {\tt ROOT} is installed (directions for installation can be found elsewhere), the following environment variable should be set:
\begin{verbatim}
  > setenv ROOTSYS [top ROOT directory]
\end{verbatim}

The latest fixed release of the {\tt AmpTools} code, along with related packages and tutorials, can be cloned or downloaded as a tarball from {\tt https://github.com/mashephe/AmpTools}.  Once the source code is unpacked, set the following:
\begin{verbatim}
